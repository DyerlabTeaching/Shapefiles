---
title: "Lines, Polygons, and Shapefiles"
format: html
---
...
We can use `R` to download and unzip the file *in the current data directory* (n.b., you can do it using a browser as well).  To use `R` you need to first download them (I've set `eval=FALSE` to the chunk so it is not redownloaded each time.  Run it by hand using `CTRL/CMD + Return`).
...
Depending upon how the archives were created, they may make a sub directory or just a pile of files in the same directory.  For this example, there are one of each with the *Zoning_Districts.* set of files expanded in the current directory and the *Roads* expanded to a subfolder named *Centerlines-shp*.
...
You can see that the `geometry` object is a `LINESTRING` (in `sf` terms).  We can see the coordinates for one of these (say *Dwyer St*), by converting the `geometry` object to a Well Known Text (WKT) version representing the sequence of points.
...
head( va_counties )
```
To get an idea of what these data represent visually, let's first plot it as a `geom_point()` object.  This will show you where all the coordinates are located (just not the connecting lines).
...
What is hidden here is the complexity of the points themselves.  Each county is identified by a `group` in the `data.frame`.




If we look at a particular county, it may be a bit more informative on how these things are constructed.  Here are the points (in red) and the underlying connecting lines creating the polygon (in grey).  
...
```
Notice it is in `CRS = NAD83/Virginia South (ftUS)`, which if we look at [epsg.io](http://epsg.io/32147) and search for it relates to EPSG=32147.  Let's do some pre-processing[^1]:  
...
<mark>We can use *spatial joins* to select features either directly.</mark>  Here I'll use the target polygon in `theFan`.
...
We can get the names of these roads using normal `dplyr` routines,
